...
const ManageProposalsPage = () => {
    ...
    // ✅ UPDATED FUNCTION: Rejects the entry in backend DB and immediately mutates local UI state
    const handleRejectClick = async (proposalId) => {
        ...
        try {
            ...
            if (response.ok) {
                // Instantly update the state array to shift this specific item's status to "rejected"
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-6">
                    <div className="flex flex-col gap-2">
                        ...
                        <p className="text-zinc-400 text-sm">
                            Review and process pitches submitted by freelancers for your active listings.
                        </p>
                    </div>

                    <div className="bg-zinc-900/60 border border-white/5 px-4 py-2.5 rounded-xl self-start sm:self-center">
                        <span className="text-xs text-zinc-500 font-medium">Total Submissions:</span>{' '}
                ...
                {proposals.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        {proposals.map((proposal) => {
                            ...
                            // Determine computed status states securely
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {/* Right Section: Functional Action Control Interface / Badges */}
                                    <div className="flex sm:flex-row lg:flex-col justify-end lg:justify-center items-center gap-3 border-t lg:border-t-0 lg:border-l border-white/5 pt-4 lg:pt-0 lg:pl-6 min-w-[180px]">
                                        ...
                                        {/* Render UI elements conditionally based on clean state evaluations */}
                                        {isAccepted ? (
                                            ...
                                            <>
                                                ...
                                                {/* Reject Offer Action Button */}
                                                <button
                                                    onClick={() => handleRejectClick(proposal._id)}
                                                    className="w-full sm:w-auto lg:w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white border border-white/5 text-xs font-bold uppercase tracking-wider rounded-xl transition-colors"
                                                >
