...
const PublicJobComponentDetails = ({ params }) => {
    ...
    // 1. ⚙️ Define all state hooks at the top
    ...
    // 2. 🎯 Define derived evaluations after states are ready
    const isClientOrAdmin = userRole === 'client' || userRole === 'admin';

    // Safe to access now that `task` is initialized by useState
    const isTaskOwner = activeUserEmail && (
        task?.clientEmail === activeUserEmail ||
        task?.client_email === activeUserEmail
    );

    const canModifyTask = userRole === 'admin' || (userRole === 'client' && isTaskOwner);
    ...
    useEffect(() => {
        ...
        const fetchTaskDetails = async () => {
            try {
                ...
                // Fetching token for protected routes
...
export default PublicJobComponentDetails;